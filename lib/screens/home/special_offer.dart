...
class _SpecialOffersState extends State<SpecialOffers> {
  ...
  itemCount: controller.homeSpecialOffers.length,
  itemBuilder: (context, index, realIndex) {
    final data = controller.homeSpecialOffers[index];
  ...
  options: CarouselOptions(
    ...
    enlargeCenterPage: true, // zoom effect on center item
    enableInfiniteScroll: true,
    autoPlay: true,
    autoPlayInterval: const Duration(seconds: 3),
    autoPlayAnimationDuration: const Duration(milliseconds: 800),
    onPageChanged: (index, reason) {
      controller.selectOfferIndex.value = index;
    },
  ),
)
          ),
        ...
        GridView.builder(
          ...
          itemBuilder: ((context, index) {
            final data = controller.  homeCategories[index];
  ...
  Widget _buildPageIndicator() {
    ...
    for (int i = 0; i < controller.  homeCategories.length; i++) {
      list.add(i == controller.  homeCategories ? _indicator(true) : _indicator(false));
    }
    return Container(
      height: 250,
